...
train_data_awake = np.array(data_awake, dtype='float32')
print('train_data_awake的维度：', train_data_awake.shape)

# train_data_awake的维度： (13, 2048, 90)
train_data_awake = train_data_awake.transpose(2,1,0)
print('train_data_awake的维度：',train_data_awake.shape)
 #train_data_awake = np.reshape(train_data_awake,(-1,2048))
 #print('train_data_awake的维度：',train_data_awake.shape)

# train_data_awake的维度： (180, 2048, 13)
train_data_sleep = np.array(data_sleep,dtype = 'float32')
train_data_sleep = train_data_sleep.transpose(2,1,0)
#train_data_sleep = np.reshape(train_data_sleep,(-1,2048))

height, width,length = train_data_awake.shape
train_input = np.zeros([height*16, 256,length])
print('train_input的维度：',train_input.shape)
train_output = np.arange(height*16)
...
for j in range(8):
    ...
    for i in range(height) :
        for j in range(length):
            # train_input[i + s, :, j] = signal.filtfilt(b, a, train_data_awake[i, s1:s2, j])
            train_input[i + s, :, j] =train_data_awake[i, s1:s2, j]
        train_output[i+s] = 1
...
for j in range(8):
    ...
    for i in range(height) :
        for j in range(length):
            # train_input[i+s,:,j] = signal.filtfilt(b, a, train_data_sleep[i,s1:s2, j])
            train_input[i + s, :, j] =train_data_sleep[i,s1:s2, j]
        train_output[i+s] = 0
